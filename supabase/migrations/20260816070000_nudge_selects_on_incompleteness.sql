-- Nudge the incomplete, not the merely unpublished.
--
-- Four drafts were published by hand today to fill the shop window. The
-- nudge's old condition (`status = 'draft'`) read that as "finished": the
-- four most incomplete visible profiles on the site became the only ones
-- the reminder could not reach. The condition now asks what the reminder
-- actually cares about -- can she submit? -- so a published-but-incomplete
-- profile keeps getting told what is missing, and a completed one falls out
-- of the audience by itself the moment the last field lands.
...
begin
  ...
  select coalesce(jsonb_agg(row_to_json(r)), '[]'::jsonb) into result
    from (
      ...
      -- Selecting on `draft` was a proxy that broke the day profiles started
      -- being published by hand: a nanny put in the shop window with six
      -- fields missing stopped being nudged at the exact moment she became
      -- the person who most needed to hear what was missing. Incompleteness
      -- is the real condition, so ask the same function that decides whether
      -- the submit button works. Approved and rejected stay out: a rejected
      -- nanny is owed an explanation, not an invitation to finish.
      ...
      select u.id, u.email, coalesce(u.first_name, 'there'), 'nudge_nanny', 0, 0,
       ...
       where cfg.audience = 'everyone'
         ...
         and u.status = 'active'
         and u.created_at < now() - make_interval(hours => cfg.nudge_after_hours)
         and (n.id is null or n.status not in ('approved', 'rejected'))
         and (n.id is null
