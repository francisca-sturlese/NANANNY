...
export function NotificationBell({
  ...
  /**
   * Whether the socket is actually carrying events.
   *
   * Not shown to anybody. It is on the element as `data-live` because the
   * difference between "subscribed" and "connected but subscribed to nothing"
   * is invisible from the outside, and both look exactly like a quiet account.
   * The end to end test waits for this before it makes something happen, which
   * is the only way that test asserts realtime rather than asserting a sleep.
   */
  ...
  /** Refetches, and reports the count it found. */
  const refresh = useCallback(async (): Promise<number | null> => {
    ...
    if (typeof feed.unread !== "number") return null;
  ...
  async function openPanel() {
    ...
    /**
     * The count comes back from the refetch rather than being read off state.
     *
     * `unread` in this scope is the value from the render that produced the
     * click, and the refetch above has just replaced it. Reading the old one
     * meant a notification that arrived while the tab was closed was shown,
     * then left unread forever: the badge said one, the panel had been opened,
     * and nothing marked it.
     *
     * The badge clears immediately rather than after the round trip, while the
     * rows keep their unread mark for this one viewing, so what is new is still
     * visible while it is being read.
     */
    const found = await refresh();
    const count = found ?? unread;
